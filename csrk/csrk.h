...
namespace detail {
...
#ifdef _MSC_VER
        //msvc doesn't allow aligned allocations to be used with free.
    ...
    template<typename T>
    [[nodiscard]]
    T* aligned_alloc_of(std::size_t alignment, std::size_t size){
        return reinterpret_cast<T*>(aligned_alloc(alignment, sizeof(T) * size));
    }
}

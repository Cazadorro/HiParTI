 ...
 * efficient manner Author: Humayun Kabir ( kabir@psu.edu ) */
...
#pragma omp simd simdlen(16)
  ...
  {
#ifdef __clang__
#pragma message "*** Kernel Used: Clang ***"
    if (NNZ > 12500000) {
#pragma omp parallel for schedule(runtime)
      for (int i_sup_row = 0; i_sup_row < numCoarsestRows; i_sup_row++) {
        ...
        const unsigned int end_index =
            mapCoarseToFinerRows[k - 1][i_sup_row + 1];
        if (likely(i_sup_row < numCoarsestRows - 1)) {
          __builtin_prefetch(&r_vec[mapCoarseToFinerRows[k - 1][i_sup_row + 1]],
                             0, 3);
          __builtin_prefetch(&r_vec[mapCoarseToFinerRows[k - 1][i_sup_row + 2]],
                             0, 3);
        }
        ...
        for (uint64_t rowIndex = start_index; rowIndex < end_index;
             rowIndex++) {
          temp_y = 0;
          if (likely(rowIndex < end_index - 8)) {
            __builtin_prefetch(&val[r_vec[rowIndex + 8]], 0, 0);
            __builtin_prefetch(&c_vec[r_vec[rowIndex + 8]], 0, 0);
          }
...
#else
#pragma message "*** Kernel Used: ICC ***"
#pragma omp parallel for schedule(runtime)
    for (int i_sup_row = 0; i_sup_row < numCoarsestRows; i_sup_row++) {
      const unsigned int start_index = mapCoarseToFinerRows[k - 1][i_sup_row];
...
#pragma omp parallel for schedule(runtime)
    for (unsigned int supSupRowIndex = 0; supSupRowIndex < numCoarsestRows;
      ...
      for (unsigned int supRowIndex = startSupRow; supRowIndex < endSupRow;
        ...
        const uint64_t start_index = mapCoarseToFinerRows[k - 2][supRowIndex];
        const uint64_t end_index = mapCoarseToFinerRows[k - 2][supRowIndex + 1];
        ...
        for (uint64_t rowIndex = start_index; rowIndex < end_index;
             rowIndex++) {
          temp_y = 0;
          if (r_vec[rowIndex + 1] - r_vec[rowIndex] < 16)
#pragma vector always
#pragma vector aligned vectorlength(4)
            for (nnz_index = r_vec[rowIndex]; nnz_index < r_vec[rowIndex + 1];
                 nnz_index++)
              temp_y += val[nnz_index] * x_test[c_vec[nnz_index]];
          else
...
CSRk_Graph::CSRk_Graph(long nRows, long nCols, long nnz, unsigned int *rVec,
  ...
  NNZ = nnz;
   ...
   r_vec = detail::aligned_alloc_of<unsigned int>((64), (N + 1));
  // r_vec = (unsigned int *) hbw_malloc((N+1)*sizeof(unsigned int));
  if (r_vec == NULL) {
    cout << "Memory can't be allocated for: r_vec. " << endl
         << "Exiting." << endl
         << endl;
  }
  c_vec = detail::aligned_alloc_of<unsigned int>((64), (NNZ));
  // c_vec = (unsigned int *) hbw_malloc(NNZ*sizeof(unsigned int));
  if (c_vec == NULL) {
    cout << "Memory can't be allocated for: c_vec. " << endl
         << "Exiting." << endl
         << endl;
  }
  val = detail::aligned_alloc_of<float>((64), (NNZ));
...
#pragma omp barrier
  ...
  x_test = detail::aligned_alloc_of<float>((64), (N));
  // x_test = (float *) hbw_malloc(N*sizeof(float));

  if (kernelType == "SpMV" && k > 1) {
    permBigG = new unsigned int[nRows];
    mapCoarseToFinerRows = detail::aligned_alloc_of<unsigned int *>((64), ((k + 1)));
    // mapCoarseToFinerRows = (unsigned int **) hbw_malloc((k+1)*sizeof(unsigned
    // int*));
    ...
    supRowSizes = detail::aligned_alloc_of<unsigned int>((64), (k - 1));
    // supRowSizes = (unsigned int *) hbw_malloc((k-1)*sizeof(unsigned int));
...
CSRk_Graph::~CSRk_Graph() {
  ...
  if (r_vec != NULL)
    detail::aligned_free(r_vec);

  if (c_vec != NULL)
      detail::aligned_free(c_vec);

  if (val != NULL)
      detail::aligned_free(val);

  if (permBigG != NULL)
    delete[] permBigG;

  if (x_test != NULL)
    detail::aligned_free(x_test);

  if (mapCoarseToFinerRows != NULL) {
      //you can't hbw free this anyway
    for (int i = 0; i < k; i++) {
      if (mapCoarseToFinerRows[i] != NULL)
        detail::aligned_free(mapCoarseToFinerRows[i]);
    }

      detail::aligned_free(mapCoarseToFinerRows);
  ...
  if (supRowSizes != NULL)
      detail::aligned_free(supRowSizes);
...
void BAND_k::stsPreprocessingWithMatching(CSRk_Graph &csrkGraph) {
  ...
  for (int i = 1; i < k; i++) {
    ...
    // csrkGraph.mapCoarseToFinerRows[i] This maps super rows from
    // smallGraphs[i] to rows in smallGraphs[i-1]
    coarsenTheGraph(i, (int)csrkGraph.supRowSizes[i - 1], supNodeSize,
                    csrkGraph);
...
int rcm_reordering(int num_verts, int *num_edges, int *adj_list, int root,
  ...
  // Now do the BFS
  std::vector<int> copy_mask(num_verts);
...
void BAND_k::randomMatching(C_GRAPH g, unsigned int *perm,
  ...
  //ideally wouldn't be static but adding as static so that we don't pay the cost every time of initializing this.
   ...
   static std::mt19937 gen(rd());
    ...
    // replaced this, this is deprecated
